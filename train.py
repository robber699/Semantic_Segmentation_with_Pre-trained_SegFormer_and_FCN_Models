...
def train(args):
    ...
    #pretrained_fcn_resnet50=fcn_resnet50(weights=None)   
    pretrained_fcn_resnet50=fcn_resnet50(pretrained=True, weights_backbone=ResNet50_Weights.DEFAULT)#.IMAGENET1K_V2
                                                                                                    #no i think    weights=FCN_ResNet50_Weights.COCO_WITH_VOC_LABELS_V1
    #modify the classifier to match the number of clas
    pretrained_fcn_resnet50.classifier[4] = torch.nn.Conv2d(512, num_classes, kernel_size=(1, 1), stride=(1, 1))

    model = DeepSegmenter(pretrained_fcn_resnet50)
    model.to(device)
...
if __name__ == "__main__":
    ...
    args.learning_rate =  0.01                                 #0.001 #0.03         #0.06
